...
public class TreeCuttingService
{
    ...
    public Task<List<MasterOptionDto>> GetZonesAsync()
    {
        var zones = Enumerable.Range(1, 10)
            .Select(i => new MasterOptionDto(i, $"Zone {i}"))
            .ToList();
        return Task.FromResult(zones);
    }

    public Task<List<MasterOptionDto>> GetPethsAsync()
    {
        var peths = Enumerable.Range(1, 15)
            .Select(i => new MasterOptionDto(i, $"Peth {i}"))
            .ToList();
        return Task.FromResult(peths);
    }

    public Task<List<MasterOptionDto>> GetPrabhaksAsync()
    {
        var prabhags = Enumerable.Range(1, 20)
            .Select(i => new MasterOptionDto(i, $"Prabhag {i}"))
            .ToList();
        return Task.FromResult(prabhags);
    }
    ...
    public async Task<ApplicationDetailDto> CreateApplicationAsync(ApplicationCreateRequest request)
    {
    ...
    public async Task<ApplicationDetailDto?> GetApplicationByIdAsync(int applicationId)
    {
        ...
        return MapToDetailDto(application, applicationTypeName, applicantTypeName, documents, documentTypeNames);
    }
